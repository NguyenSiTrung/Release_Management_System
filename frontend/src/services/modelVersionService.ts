...
import { ModelVersion, ModelVersionDetail, ModelVersionCreate, ModelVersionUpdate } from '../types';
...
export const getModelVersions = async (langPairId: number): Promise<ModelVersion[]> => {
  const response = await api.get(BASE_URL, { params: { lang_pair_id: langPairId } });
